...
function AnalizatorCertificateNew() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, "analiz_certificate"), newIkCertificate);
      ...
      {isSubmitting ? (
        ...
        <div className="grid grid-cols-1 min-h-screen w-full">
          <div className="bg-[url('https://picsum.photos/1000/1300')] bg-cover bg-no-repeat bg-center lg:bg-none grid place-items-center">
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text">
                      Газ анализатор сертификати рақами
                    </span>
                  </label>
                  <input
                    name="number"
            ...
            <Link to="/analizcertificate">Орқага</Link>
          </div>
        </div>
      )}
    </>
...
export default AnalizatorCertificateNew;
